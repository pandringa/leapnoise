...
var VolumeFilter = {
  filterType: "swipe",
  filterApplication: "iTunes", // Default to iTunes
  filterParameters: {
    state: "stop",
  ...
  filter: function(gesture) {
    ...
    if (dominantMovement.direction == "y") {
      if (this.tracking) {
        ...
        AppleScriptInterface.exec('tell application "' + this.filterApplication + '" to set sound volume to (sound volume + ' + volumeAmount + ')');
